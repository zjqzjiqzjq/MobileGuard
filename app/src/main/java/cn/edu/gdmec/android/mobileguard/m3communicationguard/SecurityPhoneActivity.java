    ...
    private void initView(){
        ...
        mListView.setOnScrollListener(new AbsListView.OnScrollListener(){
            ...
            public void onScrollStateChanged(AbsListView absListView,int i){
                switch (i){
                    case AbsListView.OnScrollListener.SCROLL_STATE_FLING: //列表滑动后静止
                        int lastVisiblePosition = mListView.getLastVisiblePosition();   //获取最后一个可见条目
                        if(lastVisiblePosition == pageBlackNumber.size() - 1){ //如果当前条目是最后一个，增查询更多的数据
                            pagenumber++;
    ...
    public void onClick(View view){
        switch (view.getId()){
            case R.id.imgv_leftbtn:
                finish();
                break;
            case R.id.btn_addblacknumber:  //跳转至添加黑名单页面
